...
namespace detail {
  ...
  public:
    ...
    std::optional<Property> getProperty(const std::vector<std::string> &propPath) {
    ...
    std::optional<Property> getProp(const std::vector<std::string> &path) {
      if (!path.empty()) {
        try {
          ...
          NEXUS_LOG(NEXUS_STATUS_NOTE, "  Properties.getProp - " << tail << " - " << typeId);
...
// Get sub-node
std::optional<Property> Properties::getProperty(const std::vector<std::string> &path) const {
  return get()->getProperty(path);
}
