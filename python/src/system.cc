...
void pynexus::init_system_bindings(py::module &m) {
  ...
  make_object_class<Command>(m, "_command")
      ...
      //                    Buffer buf) { 
      //   std::cout << "set_arg buffer " << index << std::endl;                  
      //   return self.setArgument(index, buf); }, py::arg("index"), py::arg("buf").noconvert())
      // .def("set_arg",
...
// std::string type_name = py::str(py::type::of(value).attr("__name__"));
//        std::cout << "Type as string: " << type_name << std::endl;
//             //  if (value.is_none()) {
      ...
      .def("set_arg", [](Command &self, int index, py::object value) -> nxs_status {
        ...
        // Test for bool (check before int, since bool is subclass of int in Python)
         ...
         // nxs_bool* ptr = new nxs_bool(value.cast<nxs_bool>());
          //return self.setScalar(index, static_cast<void*>(ptr));
          return self.setArgument(index, val);
        }
        // Test for int
        else if (py::isinstance<py::int_>(value)) {
          nxs_int val = value.cast<nxs_int>();
          //nxs_int* ptr = new nxs_int(val);
          //std::cout << "Set int arg index " << index << " value " << val << " ptr " << ptr << std::endl;
          //return self.setScalar(index, static_cast<void*>(ptr));
          return self.setArgument(index, val);
        }
        // Test for float
          ...
          //nxs_float* ptr = new nxs_float(val);
          //std::cout << "Set float arg index " << index << " value " << val << " ptr " << ptr << std::endl;
          //return self.setScalar(index, static_cast<void*>(ptr));
          return self.setArgument(index, val);
        }
        return NXS_Success;
